...
// Extract values from medical report for disease prediction
exports.extractValuesForPrediction = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: 'No image file provided' });
    }

    console.log('📸 Extracting values from medical report...');
    console.log('📁 File info:', { name: req.file.filename, path: req.file.path, size: req.file.size });
    console.log('🔗 Sending to ML server:', `${ML_API_URL}/extract/medical-values`);

    // Send to ML server for OCR and value extraction
    const formData = new FormData();
    const fileStream = fs.createReadStream(req.file.path);
    formData.append('file', fileStream);

    console.log('📤 Starting ML request...');
    const mlResponse = await axios.post(`${ML_API_URL}/extract/medical-values`, formData, {
      headers: formData.getHeaders(),
      timeout: 120000
    });

    console.log('✅ Values extracted:', mlResponse.data);
    ...
    res.json({
      ...
      extractedValues: {
        diabetes: {
          pregnancies: mlResponse.data.pregnancies || '',
          glucose: mlResponse.data.glucose || '',
          blood_pressure: mlResponse.data.blood_pressure || '',
          skin_thickness: mlResponse.data.skin_thickness || '',
          insulin: mlResponse.data.insulin || '',
          bmi: mlResponse.data.bmi || '',
          diabetes_pedigree: mlResponse.data.diabetes_pedigree || '',
          age: mlResponse.data.age || ''
        },
        heart: {
          age: mlResponse.data.age || '',
          sex: mlResponse.data.sex || '',
          chest_pain: mlResponse.data.chest_pain || '',
          resting_bp: mlResponse.data.resting_bp || mlResponse.data.blood_pressure || '',
          cholesterol: mlResponse.data.cholesterol || '',
          fasting_blood: mlResponse.data.fasting_blood || '',
          resting_ecg: mlResponse.data.resting_ecg || '',
          max_heart_rate: mlResponse.data.max_heart_rate || '',
          exercise_induced: mlResponse.data.exercise_induced || '',
          old_peak: mlResponse.data.old_peak || ''
        }
      },
      rawData: mlResponse.data
    });
  } catch (error) {
    // Clean up uploaded file on error
    if (req.file) {
    ...
    console.error('❌ Value extraction error:', error.message);
    console.error('📋 Full error details:', {
      message: error.message,
      code: error.code,
      status: error.response?.status,
      mlServerData: error.response?.data,
      timeout: error.code === 'ECONNABORTED' ? 'Request timeout' : 'N/A'
    });

    res.status(500).json({
      message: 'Error extracting values from report',
      error: error.message,
      mlError: error.response?.data || error.code,
      hint: 'Make sure ML server is running and the document is clear'
    });
  }
};
