      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        {!loading && !error && entries.length === 0 && (
          <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'var(--color-surface)', borderRadius: 'var(--radius-card)', border: '2px dashed var(--color-border)' }}>
            ...
            <p style={{ color: 'var(--color-text-muted)', margin: 0, fontSize: '0.9rem' }}>
              No history yet. Upload your first report to start building your timeline.
            </p>
          </div>
        )}
        ...
        {!loading && !error && entries.length > 0 && (
          <div style={{ position: 'relative' }}>
            ...
            {entries.map((entry, i) => (
              <div key={entry.reportId || i} style={{ display: 'flex', gap: '1.25rem', marginBottom: '1.5rem', position: 'relative', zIndex: 1 }}>
                {/* Date column */}
                ...
                >
                  ...
                  {entry.reportId && (
                    <Link to={`/report/${entry.reportId}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', marginTop: '0.75rem', fontSize: '0.8rem', color: 'var(--color-accent)', fontWeight: 600, textDecoration: 'none' }}
